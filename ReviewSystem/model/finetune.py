...
class WBDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        token = self.tokenizer(
            content, padding="max_length", truncation=True, max_length=140
        )
...
class Trainer:
    ...
    def train_epoch(self, epoch):
        ...
        for index, (input_ids, token_type_ids, attention_mask, label) in pbar:
            ...
            train_f1_score = f1_score(
                y_true=label.cpu().numpy(), y_pred=preds, average="macro"
            )
            ...
            pbar.set_description(
                f"{train_loss.avg: .3f}  {acc.avg: .3f}  {f1.avg: .3f}"
            )
            pbar.set_postfix(
                {
    ...
    def _val(self):
        ...
        val_f1_score = f1_score(
            y_true=np.array(labels), y_pred=np.array(preds), average="macro"
        )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Bert fine-tune on SMP2020-EWECT Dataset"
    )
    parser.add_argument(
        "--model_name",
    ...
    parser.add_argument(
        "--num_labels", default=6, type=int, help="fine-tune num labels"
    )
    parser.add_argument(
        "--train_data_path",
        default="data_set/train/usual_train.txt",
        type=str,
        help="train data path",
    )
    parser.add_argument(
        "--val_data_path",
        default="data_set/eval_data/usual_eval_labeled.txt",
        type=str,
        help="train data path",
    )
    parser.add_argument(
        "--batch_size", default=64, type=int, help="train and validation batch size"
    )
    parser.add_argument(
        "--dataloader_num_workors",
        default=8,
        type=int,
    ...
    parser.add_argument(
        "--output_dir", default="workspace/wb", type=str, help="save dir"
    )
    parser.add_argument(
        "--save_model_iter",
        default=5,
        type=int,
        help="save model num epochs on training",
    )
