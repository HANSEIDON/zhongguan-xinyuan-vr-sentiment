...
def parse_args():
    ...
    parser.add_argument('--model_path', default='/content/drive/MyDrive/workspace/wb/best.pt',
                        type=str, help='model checkpoint path')
    parser.add_argument('--num_labels', default=6, type=int,
                        help='number of labels for classification')

    parser.add_argument('--max_length', default=140, type=int,
...
def main():
    ...
    label_map = {0: 'happy', 1: 'angry', 2: 'sad', 3: 'fear', 4: 'surprise', 5: 'neutral'}
    pred_label = label_map.get(pred_id, f'unknown({pred_id})')
    emoji_map = {'happy':'😊','angry':'🤬','sad':'😢','fear':'😱','surprise':'😧','neutral':'😑'}
    emoji_label = emoji_map.get(pred_label,f'unknown({pred_label})')
    print(f'\n입력 문장: {args.input}')
    print(f'예측 라벨: {emoji_label} (id={pred_id})')
